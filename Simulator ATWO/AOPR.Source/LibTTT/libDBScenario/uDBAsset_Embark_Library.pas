...
uses  tttData, Windows, uCoordConvertor, Classes, uBaseCoordSystem, uDataTypes,
      Graphics, uLibSettingTTT, uFormula, uSnapshotData;
...
type
  ...
  public
    ...
    function GetSnapshotData : _SS_TPlatformBase;
...
function TPlatformBase.GetSnapshotData: _SS_TPlatformBase;
var
  ssPlatformBaseTemp : _SS_TPlatformBase;
begin
  ssPlatformBaseTemp.PlatformBaseId := id;
  ssPlatformBaseTemp.Identifier := identifier;
  ssPlatformBaseTemp.force := force;
  ssPlatformBaseTemp.Longitude := Longitude;
  ssPlatformBaseTemp.Latitude := Latitude;
  ssPlatformBaseTemp.Radius := Radius;
  ssPlatformBaseTemp.TypeBase := TypeBase;
  // ListVehicle : Tlist;
  // ListPersonel : Tlist;
  //  isShow : Boolean;
  // isSelected : Boolean;

  ssPlatformBaseTemp.Fuel := Fuel;
  ssPlatformBaseTemp.Lubricants := Lubricants;
  ssPlatformBaseTemp.FreshWater := FreshWater;
  ssPlatformBaseTemp.Water := Water;
  ssPlatformBaseTemp.Food := Food;

  //  Lubricants, FreshWater, Water, Food : Double; {dlm m3, kecuali food dlm ton}
  result := ssPlatformBaseTemp;
end;
