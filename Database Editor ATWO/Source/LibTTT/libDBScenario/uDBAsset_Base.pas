...
uses  tttData, Windows, uCoordConvertor, Classes, uBaseCoordSystem, uDataTypes,
      Graphics, uLibSettingTTT, uFormula,  uDBEditSetting;
...
begin
  ...
  begin
    ...
    if FData.Base_TypeBase = 0 then
      basebmp.LoadFromFile(vAppDBSetting.BMPPath + pctBaseAU + '.bmp')
    else
      basebmp.LoadFromFile(vAppDBSetting.BMPPath + pctBaseAL + '.bmp');
