...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.Imaging.pngimage, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.OleCtrls,
  MapXLib_TLB, System.ImageList, Vcl.ImgList, Vcl.ComCtrls, Vcl.ToolWin,
  uCoordConvertor, uBaseCoordSystem, uSimContainers, uDBAssetObject, uDBAsset_Deploy,
  uDBAsset_GameEnvironment, uDBEditSetting, RzBmpBtn ;
...
uses
  uDataModuleTTT, ufrmSummaryScenario, uDBAsset_Base, newClassASTT ;
...
begin
  ...
  repeat
    snInfo := TSnapshot_Definition.Create;
...
//  btnMove.Picture.LoadFromFile('dataATWO\Image DBEditor\Interface\Button\btnMove_Select.PNG');
end;
...
//  btnMove.Picture.LoadFromFile('dataATWO\Image DBEditor\Interface\Button\btnMove_Normal.PNG');
...
begin
  ...
  lBearingFCenter.Caption := FormatFloat('0.00',
    CalcBearing(FSelectedEnvironment.FGameArea.Game_Centre_Long, FSelectedEnvironment.FGameArea.Game_Centre_Lat, dx, dy));

  lDistanceFCenter.Caption := FormatFloat('0.00',
    CalcRange(FSelectedEnvironment.FGameArea.Game_Centre_Long, FSelectedEnvironment.FGameArea.Game_Centre_Lat, dx, dy));
...
var
  i : Integer;
  snapshotTemp : TSnapshot_Definition;
begin
  lstSnapshot.Clear;
...
procedure TfrmExercise.PageControllChange(Sender: TObject);
begin
