...
type
  ...
  private
    ...
    FPlatInstList : TList;
    FAirVehicleList : TList;
    FSelectedAirVehicle : TVehicle_Definition;
  public
    { Public declarations }

    procedure UpdateTabPage;
    procedure UpdateModelPanel;
    procedure UpdateAirVehicleList;
    procedure UpdateAirVehicleData;
    procedure UpdateMotionData;
    procedure UpdatePIIdentData;
  end;

var
  frmAirView: TfrmAirView;
...
procedure TfrmAirView.FormClose(Sender: TObject; var Action: TCloseAction);
begin
//  FreeItemsAndFreeList(FAirVehicleList);
//
//  Action := cafree;
end;

procedure TfrmAirView.FormCreate(Sender: TObject);
begin
  FAirVehicleList := TList.Create;
end;

procedure TfrmAirView.FormDestroy(Sender: TObject);
begin
  FreeItemsAndFreeList(FAirVehicleList);
end;

procedure TfrmAirView.FormShow(Sender: TObject);
begin
  tabIndex := 0;
  tabMax := 7;
...
procedure TfrmAirView.ImgBtnBackClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmAirView.ImgBtnNextTabClick(Sender: TObject);
begin
  tabIndex := tabIndex + 1;
  UpdateTabPage
end;

procedure TfrmAirView.ImgBtnPreviousTabClick(Sender: TObject);
begin
  tabIndex := tabIndex - 1;
  UpdateTabPage
end;

procedure TfrmAirView.ImgBtnNextModelClick(Sender: TObject);
begin
...
procedure TfrmAirView.ImgBtnPreviousModelClick(Sender: TObject);
begin
  modelIndex := modelIndex - 1;
  UpdateModelPanel
end;

procedure TfrmAirView.lbSingleClick(Sender: TObject);
begin
  if lstAir.ItemIndex = -1 then
    Exit;

  FSelectedAirVehicle := TVehicle_Definition(lstAir.Items.Objects[lstAir.ItemIndex]);

  UpdateAirVehicleData;
end;

procedure TfrmAirView.btnAssetClick(Sender: TObject);
begin
  if Assigned (FSelectedAirVehicle) then
  begin
    frmAssetDetail := TfrmAssetDetail.Create(Self);
    try
    with frmAssetDetail do
    begin
      SelectedVehicle := FSelectedAirVehicle;
...
procedure TfrmAirView.UpdateAirVehicleData;
var
  i : Integer;
...
procedure TfrmAirView.UpdateAirVehicleList;
var
  i : Integer;
  airvehicle : TVehicle_Definition;
begin
  ...
  dmTTT.GetFilterVehicleDefByDomain(FAirVehicleList, Ord(vhdAir));
...
procedure TfrmAirView.UpdateMotionData;
var
  motion : TMotion_Characteristics;
begin
  with FSelectedAirVehicle.FData do
  begin
...
procedure TfrmAirView.UpdateCbbCategoryItems;
begin
  with FSelectedAirVehicle do
  begin
    case FData.Platform_Domain of
...
procedure TfrmAirView.UpdateCbbTypeItems;
begin
  with FSelectedAirVehicle.FData do
  begin
    {$REGION ' Air '}
...
procedure TfrmAirView.UpdatePIIdentData;
var
  i : Integer;
  PIIdentifer : TPlatform_Instance_Identifier;
begin
  dmTTT.GetAllPlatformInstanceIdentifier(FSelectedAirVehicle.FData.Vehicle_Index, FPlatInstList);
...
procedure TfrmAirView.UpdateModelPanel;
var
  i : Integer;
  PIIdentifer : TPlatform_Instance_Identifier;
begin
...
procedure TfrmAirView.UpdateTabPage;
begin
